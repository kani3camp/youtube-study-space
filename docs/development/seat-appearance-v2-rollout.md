...
2. 配信で使用する全 monitor / OBS browser source の DOM を確認し、`html` 要素の `data-seat-appearance-schema-read` が `1,2` であることを確認する。
3. V2 canonical field と V1 legacy field を dual-write する backend をデプロイする。
4. backend の起動ログで `seat-appearance-schema-write=2` と `seat-appearance-legacy-write=true` を確認する。
...
- 稼働中の全 backend instance の起動ログで V2 dual-write capability を確認できる。
- 配信で使用する全 frontend / OBS browser source で V1/V2 dual-read capability を確認できる。
