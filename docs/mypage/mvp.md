...
| Firebase access token / YouTube access token | 認証情報をログに混入させないため        |
...
2. `y`GET /mypage/me` で Firebase UID と YouTube channel ID の対応関係が存在しない場合は、`401`ではなく`409 link_required` を返す。
...
その後、取得した YouTube access token を使って `POST /mypage/auth/youtube-link` を呼び出し、サーバー側マッピングを作成・更新する。ID に紐づけて保存する
...
フロントエンドは `409 link_required` を受け取った場合、保持済みの YouTube access token に依存せ### `GET /mypage/me` レスポンス型
...
type MyPageMeSuccessResponse =
  | MyPageOkResponse
  | MyPageNotRegisteredResponse access token を取り直す。
...
interface MyPageOkResponse {
  ...
  currentSeainterface MyPageNotRegisteredResponse {
...
| `GET /mypage/me`      | `403` / `429` / `500` / `502` | `MyPageErrorResponse`     | 認可失敗、レート制限、内部oolean           |
| state: "work"         | "break"                       |                           |                               |
| workName: string      |                               |                           |                               |
