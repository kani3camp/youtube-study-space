...
引数なしで、上から4テーマ行（各 `data/0N_*.txt` から1行を独立に乱数抽選）に加え、**座席数 10〜15** を1回一様乱数で決定し、それと `data/prompt_template.txt` を連結した UTF-8 テキストを **`output/prompt-<タイムスタンプ>.txt`** に書き込み、**保存したファイルの絶対パスを標準出力に1行**出します。実行時は **`tools/room-image-prompt` をカレントディレクトリにした状態**で動かす想定です（省略時の出力先が `./output/` になるため）。
