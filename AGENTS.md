...
- `youtube-monitor/` uses `pnpm` as the package manager (`packageManager: pnpm@10.4.0`).
- Do not use `npm` or `yarn` for `youtube-monitor/` unless explicitly required.
...
- Start the frontend production server with `pnpm start` in `youtube-monitor/` (port 18080)
