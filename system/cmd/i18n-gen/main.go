...
func main() {
	...
	mustWriteString(&out, "// Code generated by i18n-gen; DO NOT EDIT.\n")
	mustWritef(&out, "// baseline=%s\n", baselineLang)
	mustWritef(&out, "package %s\n\n", outputPkgName)
	mustWriteString(&out, "import (\n\t\"")
	mustWriteString(&out, i18nImportPath)
	mustWriteString(&out, "\"\n)\n\n")
	...
	for _, ok := range ordered {
		...
		mustWritef(&out, "// %s: key %q\n", funcName, ok.namespace+":"+ok.key)
		mustWritef(&out, "func %s(", funcName)
		for i, arg := range parsedArgs {
			if i > 0 {
				mustWriteString(&out, ", ")
			}
			mustWritef(&out, "%s %s", arg.name, arg.typ)
		}
		mustWriteString(&out, ") string {\n")
		mustWritef(&out, "\treturn engine.TranslateDefault(%q", ok.namespace+":"+ok.key)
		for _, arg := range parsedArgs {
			mustWriteString(&out, ", ")
			mustWriteString(&out, arg.name)
		}
		mustWriteString(&out, ")\n}\n\n")
	}
...
func mustWriteString(out *bytes.Buffer, s string) {
	if _, err := out.WriteString(s); err != nil {
		panic(fmt.Errorf("write string: %w", err))
	}
}

func mustWritef(out *bytes.Buffer, format string, args ...any) {
	if _, err := fmt.Fprintf(out, format, args...); err != nil {
		panic(fmt.Errorf("write formatted string: %w", err))
	}
}
